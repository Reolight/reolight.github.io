...
test("Передвигает 2 движимых числа на 1 позицию", () => {
    maskSynthetizer.generate(nonReqMask);
...
test("Передвигает 2 движимых символа за литерал на 2 позиции (литерал не считается позицией)", () => {
    maskSynthetizer.generate(nonReqMaskWithLiteral);
    maskSynthetizer.putSymbols("11", 0);
    maskSynthetizer['shiftOccupiedRight'](0, 2);
...
test("Передвигает 2 движимых символа на литерал: один символ перед, другой за", () => {
    maskSynthetizer.generate(nonReqMaskWithLiteral);
    maskSynthetizer.putSymbols("11", 0);
    maskSynthetizer['shiftOccupiedRight'](0, 1);
...
// test("Передвигает 1 движимый символ на 2 позиции, но НЕ упирается в букву", () => {
...
// test("Передвигает 1 движимый символ на 3 позиции, но упирается в букву", () => {
//     maskSynthetizer.generate(maskWithLetterInside);
//     maskSynthetizer.putSymbols("1", 0);
